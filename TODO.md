...
16. Dynamically switch to higher priority connection when it becomes available.
    Shipped: `InstanceReplacedAtomically` replaces a live instance in place when
    a better socket path appears (ring: Thunderbolt > Ethernet > Wi-Fi). Master
    holds the candidate for 2s via an injectable clock so enumerating interfaces
    do not flap; workers shut down runners whose bound instance no longer matches
    and recreate on the new hosts. Same-priority IP churn is ignored; a dead
    current path fails over. See `src/exo/master/connection_upgrade.py`.
