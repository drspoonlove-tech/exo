...
14. Show the type of each connection (TB5, Ethernet, etc.) in the UI. Refer to old exo: https://github.com/exo-explore/exo/blob/56f783b38dc6b08ce606b07a5386dc40dae00330/exo/helpers.py#L251
    Shipped: topology edges now label TB5 / Thunderbolt / Ethernet / Wi-Fi / RDMA / Unknown from existing `NetworkInterfaceInfo.interface_type` (plus RDMA edges and TB5 via 80/120 Gb/s link speed or "Thunderbolt 5" hardware port). Classifier no longer clobbers Thunderbolt `en*` ports to `maybe_ethernet`; Linux uses iface-name + sysfs hints. Not a bandwidth/latency profiler.
