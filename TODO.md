...
7. Continuous batching: shipped. A new prompt no longer blocks in-flight decode. When a generation batch is already running, remaining prompt tokens are inserted into mlx-lm's incremental prefill path so decode continues between prefill chunks. Pipeline-parallel and vision prompts still use the blocking custom prefill (those paths cannot be interleaved safely).
